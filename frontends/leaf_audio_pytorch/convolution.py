...
from frontends.leaf_audio_pytorch import impulse_responses
...
class GaborConv1D(nn.Module):
    ...
    def __init__(
        ...
        initialized_kernel = kernel_initializer(
            torch.zeros(self._filters, 2),
    ...
    def forward(self, x):
        ...
        if self._sort_filters:
            filter_order = torch.argsort(kernel[:, 0]).repeat(2,1)
            kernel = torch.gather(kernel, dim=0, index=filter_order.T)
        ...
        output = F.conv1d(
            x,
            stacked_filters,
            bias=self._bias if self._use_bias else None,
